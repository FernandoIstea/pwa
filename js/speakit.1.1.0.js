class Speakit extends SpeechSynthesis {
    static utteranceRate = 1
    static utterancePitch = 1
    static totalAvailableVoices = 0
    static languageFilter = "en-GB" // U can specify In this property an ISO language to force filtering the available voices, or let it empty to get the complete voices list
    static totalVoices = []
    ...
    static readText(text, lang, nameOfVoice) {
        return new Promise(function (resolve, reject) {
            ...
            if (nameOfVoice) {
                let voice 
                Speakit.getVoices().then((r)=> {
                    voice = r.find((v)=> v.name === nameOfVoice)
                    voice ? utterance.voice = voice 
                    : console.error(`The selected voice '${nameOfVoice}' is not available.`)
    ...
    static TTStest() {
        if (speechSynthesis) {
            this.readText("Your web browser has supported for using Text-To-Speech.", 
                          "en-GB", 
                          'Martha')
    ...
    static about() {
        return `Copyright 2024-02-08: Fernando Omar Luna - @ mobilepadawan.
